...
def feeds():
    ...
    response = requests.get("https://hlofcnapp2.azurewebsites.net/api/getAdvertisements")
    posts = response.json()
...
def home():
    response = requests.get("https://hlofcnapp2.azurewebsites.net/api" + '/getAdvertisements')
    response2 = requests.get("https://hlofcnapp2.azurewebsites.net/api" + '/getPosts')
...
def edit_ad_view(id):
    response = requests.get("https://hlofcnapp2.azurewebsites.net/api" + '/getAdvertisement?id=' + id)
    ad = response.json()
...
def add_ad_request():
    ...
    response = requests.post("https://hlofcnapp2.azurewebsites.net/api" + '/createAdvertisement', json=json.dumps(req_data))
...
def update_ad_request(id):
    ...
    response = requests.put("https://hlofcnapp2.azurewebsites.net/api" + '/updateAdvertisement?id=' + id, json=json.dumps(req_data))
    return redirect(url_for('home'))

@app.route('/ad/delete/<id>', methods=['POST'])
def delete_ad_request(id):
    response = requests.delete("https://hlofcnapp2.azurewebsites.net/api" + '/deleteAdvertisement?id=' + id)

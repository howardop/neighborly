...
CMD="az aks create --name $KUBECLUSTERNAME  --resource-group $RESOURCEGP --node-count 2  --generate-ssh-keys --node-vm-size standard_b2s"
echo $CMD
eval $CMD
...
az aks update -n $KUBECLUSTERNAME -g $RESOURCEGP --attach-acr $REGNAME


# Now we are ready to deploy
func kubernetes deploy --name  $KUBECLUSTERNAME --image-name $DOCKERLOGINSVR/neighborlyapp  --dry-run 
